...
//验证只能输入范围内位数的数字和字母
function isNumOrEnglish(obj, max, min) {
    reg = /^[0-9a-zA-Z]*$/g;
    if (!reg.test(obj)) {
        return false;
    } else {
...
/**
数据验证完整性（可排除某些notIds容器或控件）
**/
function CheckDataValidNotIds(id, notIds) {
    if (!JudgeValidateNotIds(id, notIds)) {
        return false;
    } else {
        return true;
    }
...
//obj为当前input所在的空间容器 (例如：Div,Panel)
//脚本中 checkvalue 验证函数  err 属性表示提示【中文名称】
function JudgeValidateNotIds(obj, notIds) {
    var Validatemsg = "";
    var Validateflag = true;
    $(obj).not(notIds).find("[datacol=yes]").each(function () {
        if ($(this).attr("checkexpession") != undefined) {
            var value = $(this).val();
            if (value == "==请选择==") {
                value = "";
